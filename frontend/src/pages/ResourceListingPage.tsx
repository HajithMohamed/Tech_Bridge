...
const ResourceListingPage = () => {
  ...
  return <div className="min-h-screen"><main className="max-w-6xl mx-auto px-4 sm:px-6 py-9">
    ...
    {allowedTypes.length === 0 ? <div className="glass-card p-7"><p className="font-semibold text-surface-900">Technical resource listings are not enabled for this provider.</p><p className="mt-2 text-sm text-gray-500">Enable Technical resources and at least one resource access pathway in your provider profile.</p><Link to="/provider/profile" className="inline-block mt-4 text-primary-600 hover:text-primary-700 font-semibold">Update provider services →</Link></div> : <form onSubmit={submit} className="glass-card p-5 sm:p-7 space-y-6">
      ...
      <div className="grid sm:grid-cols-1 gap-4"><Field label="Item name"><TextInput value={form.itemName} onChange={(itemName) => setForm((current) => ({ ...current, itemName }))} placeholder="e.g. Arduino UNO" /></Field></div>
    ...
    <section className="mt-10"><h2 className="text-xl font-bold text-surface-900 mb-4">Your listings</h2>{loading ? <p className="text-gray-500">Loading your listings...</p> : listings.length === 0 ? <div className="glass-card p-6 text-gray-500">You have not listed any resources yet.</div> : <div className="grid md:grid-cols-2 gap-4">{listings.map((listing) => <article key={listing._id} className="glass-card p-5"><div className="flex justify-between gap-3"><div><h3 className="font-semibold text-surface-900">{listing.itemName}</h3><p className="mt-1 text-xs text-gray-500">{readable(listing.accessType)} · {listing.quantityAvailable} units</p></div><select value={listing.status} onChange={(event) => void setStatus(listing, event.target.value as 'available' | 'claimed')} className="feed-input !w-auto !py-1.5 text-sm"><option value="available">Available</option><option value="claimed">Claimed</option></select></div><div className="mt-4 flex items-center justify-between"><span className={`text-xs px-2 py-1 rounded ${listing.providerOrgVerified ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>{listing.providerOrgVerified ? 'Verified provider' : 'Community listing'}</span><button onClick={() => void remove(listing._id)} className="text-sm text-red-600 hover:text-red-700">Delete</button></div></article>)}</div>}</section>
  </main></div>;
};
