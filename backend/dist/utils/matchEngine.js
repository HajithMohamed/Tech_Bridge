...
exports.SKILL_RESOURCES = void 0;
exports.matchScore = matchScore;
exports.getResourceForSkill = getResourceForSkill;
 ...
 */
const fuzzyMatch = (studentSkill, requiredSkill) => {
 ...
 */
function matchScore(studentProfile, opportunity) {
    ...
    if (studentProfile?.careerGoal) {
        ...
        const titleHits = goalWords.filter((w) => titleNorm.includes(w) || descNorm.includes(w));
        if (titleHits.length > 0) {
            careerRelevanceScore = Math.min(100, 50 + (titleHits.length / goalWords.length) * 50);
        }
    ...
    // ── WEIGHTED TOTAL ────────────────────────────────────
    const raw = skillScore * WEIGHTS.skills +
...
function getResourceForSkill(skill) {
    ...
    for (const [mapKey, resource] of Object.entries(exports.SKILL_RESOURCES)) {
