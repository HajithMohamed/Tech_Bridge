...
export const updateProviderProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (req.body.opportunityCategories !== undefined) {
      if (!Array.isArray(req.body.opportunityCategories) || !req.body.opportunityCategories.length || req.body.opportunityCategories.some((service: unknown) => typeof service !== 'string' || !providerServices.includes(service))) {
